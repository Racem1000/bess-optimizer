...
def run_optimizer(prices, capacity, max_power, efficiency, initial_soc):
    T = len(prices)
    m = ConcreteModel()
    m.T = RangeSet(0, T-1)
...
with tab2:
    ...
    charge_f, discharge_f, soc_f = run_optimizer(forecast_24, capacity, max_power, efficiency, INITIAL_SOC)
    charge_p, discharge_p, _ = run_optimizer(last_24_actual, capacity, max_power, efficiency, INITIAL_SOC)
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=hours, y=last_24_actual, name="Actual Price", line=dict(color="white", width=2)))
    fig2.add_trace(go.Scatter(x=hours, y=forecast_24, name="Forecast Price", line=dict(color="#4FC3F7", width=2, dash="dash")))
    fig2.add_trace(go.Bar(x=hours, y=[-c for c in charge_f], name="Charge (buy)", marker_color="royalblue", opacity=0.7))
    fig2.add_trace(go.Bar(x=hours, y=discharge_f, name="Discharge (sell)", marker_color="crimson", opacity=0.7))
    fig2.add_trace(go.Scatter(x=hours, y=soc_f, name="SOC (MWh)", line=dict(color="lime", width=2, dash="dot"), yaxis="y2"))
    fig2.update_layout(
...
with tab3:
    ...
    st.info("Full backtest on SE3 Nord Pool data (Mar–May 2026). ML optimizer achieved 189.5% revenue uplift vs naive dispatch strategy.")
